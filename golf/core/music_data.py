...
def period_words(rom: bytes, layout: MusicLayout) -> list[int]:
    ...
    b = _bank14(rom)
    return [
        b(layout.period_table + 2 * i) | (b(layout.period_table + 2 * i + 1) << 8)
        for i in range(13)
    ]
...
def channel_end(rom, layout, h, channel: str) -> int:
    ...
    if channel == "pulse2":
        return pulse2_end
    start = {
        "pulse1": h["pulse1_start"],
        "triangle": h["triangle_start"],
        "noise": h["noise_start"],
        "dmc": h["dmc_start"],
    }[channel]
    if start == 0:
...
def envelope_rows(
    rom: bytes, layout: MusicLayout, patterns: list[dict]
) -> dict[str, str]:
    ...
    b = _bank14(rom)
    bases = sorted(
        {p[k] for p in patterns for k in ("pulse2_envelope", "pulse1_envelope")}
    )
    return {
        f"{base:02X}": " ".join(
            f"{b(layout.envelope_table + base + i):02X}"
            for i in range(ENVELOPE_ROW_LEN)
        )
        for base in bases
    }
...
def export(
    rom: bytes, music_ids, *, source: str = "", reference: bytes | None = None
) -> dict:
    ...
    if reference is not None:
        out["engine"]["semitones_sharper_than_reference"] = semitone_offset(
            rom, reference
        )
    return out
