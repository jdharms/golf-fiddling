...
OPTIONS = PlayerOptions("LUIGI", frozenset({Club.W1, Club.W3, Club.I5, Club.PW, Club.SW}), bgm=False)
CREDENTIALS = credentials_for(839299365868340223, 0xDEADBEEF, (b"\x11" * 8, b"\x22" * 8))
...
def jp_manifest(catalog, curation):
    return generate(catalog, curation, Settings(prng_seed="build-stages-jp", music="jp_france"))


@pytest.fixture(scope="module")
def nes_manifest(catalog, curation):
    settings = Settings(prng_seed="build-stages-nes", sources={US_ROM}, music="nes_us", mercy_point=None)
...
def test_the_unfinished_build_is_deterministic(unfinished, jp_manifest, catalog, store, vanilla):
...
def test_the_unfinished_rom_holds_the_placeholder_fill(unfinished):
    for _, symbol, length in PLACEHOLDERS:
        ...
        assert unfinished.rom[start : start + length] == bytes([port.PATCH_FILL]) * length


def test_a_nes_open_seed_repoints_the_theme_and_can_leave_out_mercy(nes_manifest, catalog, store, vanilla):
    names = [step.name for step in unfinished_steps(nes_manifest, catalog, store, vanilla)]
...
def test_signed_in_finishing_changes_only_defaults_and_credentials(signed_in, unfinished, vanilla):
    assert list(signed_in.regions) == ["sram_defaults", "qr_credentials"]
    allowed = file_offsets(signed_in.regions["sram_defaults"]) | file_offsets(signed_in.regions["qr_credentials"])
    assert changed(unfinished.rom, signed_in.rom) <= allowed
    assert ips.apply(vanilla, signed_in.ips) == signed_in.rom

    values = {"seed_id": CREDENTIALS.seed_id, "player_ids": b"".join(CREDENTIALS.player_ids), "mac_keys": b"".join(CREDENTIALS.keys)}
    for suffix, symbol, length in PLACEHOLDERS:
        start = HEADER + placeholder_offset(symbol)
        assert signed_in.rom[start : start + length] == values[suffix]


def test_guest_finishing_changes_only_defaults_and_the_splice(guest, unfinished, vanilla):
...
def test_the_stages_overlap_only_where_scorecard_qr_wrote(flavour, request, unfinished):
    ...
    unfinished_bytes = set().union(*(prg_bytes(regions) for regions in unfinished.regions.values()))
    finishing_bytes = set().union(*(prg_bytes(regions) for regions in finished.regions.values()))
...
@pytest.mark.parametrize("credentials, finisher", [(CREDENTIALS, "qr_credentials"), (None, "qr_disable")])
...
def test_finishing_refuses_a_bag_the_seed_forbids(jp_manifest, vanilla, unfinished):
    strict = replace(jp_manifest, course=replace(jp_manifest.course, clubs=ClubRules(banned={Club.W1})))
