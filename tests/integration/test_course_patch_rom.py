...
def read_bank_table(rom: RomReader) -> bytes:
    return rom.read_prg(
        rom_utils.cpu_to_prg_switched(BANK_TABLE_CPU_ADDR, 3), BANK_TABLE_SIZE
    )
...
def assert_terrain_roundtrips(rom: RomReader, holes: list[HoleData]):
    ...
    for hole_idx, hole in enumerate(holes):
        ...
        compressed = rom.read_prg(
            rom_utils.cpu_to_prg_switched(start, bank), end - start
        )
...
def test_course_roundtrip(japan_holes, tmp_path):
    ...
    compressed = rom.read_prg(
        rom_utils.cpu_to_prg_switched(greens_ptr, 3), next_ptr - greens_ptr
    )
...
def test_metadata_roundtrip(japan_holes, tmp_path):
    ...
    for hole_idx, hole in enumerate(japan_holes):
        ...
        assert rom.read_fixed_byte(rom_utils.TABLE_PAR + hole_idx) == metadata.get(
            "par", 4
        )
        ...
        assert rom.read_fixed_byte(
            rom_utils.TABLE_SCROLL_LIMIT + hole_idx
        ) == metadata.get("scroll_limit", 32)
...
def test_scorecard_totals_follow_the_course(tmp_path):
    ...
    course_digits = [
        rom.read_switched(table, 2, 3) for table in (0xAF71, 0xAF74, 0xAF77)
    ]
...
def test_greens_sequential_in_bank3(japan_holes, tmp_path):
    ...
    pointers = [
        rom.read_fixed_word(rom_utils.TABLE_GREENS_PTR + i * 2) for i in range(18)
    ]
...
def test_leaves_bank_2_and_holes_18_to_53_alone(tmp_path):
    ...
    assert rom.read_prg(bank2, 0xA554 - 0x837F) == vanilla.read_prg(
        bank2, 0xA554 - 0x837F
    )
    ...
    for table, width in [
        ...
        assert rom.read_fixed(start, 36 * width) == vanilla.read_fixed(
            start, 36 * width
        )
