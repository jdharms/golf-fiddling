...
def test_vanilla_bytes_match_the_rom(vanilla):
    ...
    assert rom.read_switched(ATTRIBUTES_ADDR, BANK, len(_VANILLA_ATTRIBUTES)) == _VANILLA_ATTRIBUTES
...
def test_the_stroke_play_encoder_matches_the_rom(vanilla):
    ...
    assert rom.read_switched(0xB02C, BANK, 30) == title_descriptor_bytes("18H STROKE PLAY TOURNAMENT")
...
def test_title_on_the_rom(vanilla):
    ...
    assert rom.read_switched(TITLE_DESCRIPTOR_ADDR, BANK, 30) == title_descriptor_bytes(title)
