...
def test_vanilla_bytes_match_the_rom(vanilla):
    ...
    assert rom.read_switched(0xAD46, BANK, 10) == bytes(
        [0xA2, 0x17, 0xA9, 0xFF, 0x9D, 0x98, 0x6F, 0xCA, 0x10, 0xF8]
    )
...
def test_every_default_on_the_rom(vanilla):
    ...
    assert rom.read_switched(CLUB_BAG_ADDR, BANK, 14) == bytes(
        [0x00, 0x02, 0x08, 0x0D, 0x0E, 0x0F] + [0xFF] * 8
    )
