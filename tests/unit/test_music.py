...
def test_slugs_match_the_music_dumps():
    ...
    expected = {f"nes_{course['name']}": (US_ROM, us[course["name"]]) for course in rom_utils.COURSES}
    expected |= {
        course["name"]: (JP_ROM, jp[f"course_{slot}"])
        for slot, course in enumerate(jp_rom_utils.COURSES, start=1)
    }
    assert {slug: (theme.rom, theme.music_id) for slug, theme in TRACKS.items()} == expected
...
def test_mario_open_draws_cover_every_theme():
    rng = random.Random(1)
    assert {choose_music(rng, include_mario_open=True) for _ in range(500)} == set(TRACKS)
