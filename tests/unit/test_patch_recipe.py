...
class TestRegistry:
    def test_every_patch_type_builds_a_patch_of_its_own_name(self, credentials_file):
        ...
        for spec_id in PATCH_SPECS:
            ...
            (built,) = Recipe([step({"patch": spec_id, **params.get(spec_id, {})})]).build_steps(b"")
    ...
    def test_describe_params(self):
        ...
        assert describe_params(PATCH_SPECS["practice_swing"]) == "hold_frames (integer, default 120)"
...
class TestParams:
    def test_integers_accept_numbers_and_strings_in_any_base(self):
        ...
        assert step({"patch": "practice_swing", "hold_frames": "0x60"}).params.hold_frames == 0x60
    ...
    def test_optional_values_take_null(self):
        assert step({"patch": "mercy_tap_in", "mercy_point": 9, "mercy_result": None}).params.mercy_result is None

    def test_paths_are_relative_to_the_base_dir(self, tmp_path):
        assert step({"patch": "course", "course": "japan"}, tmp_path).params.course == tmp_path / "japan"
        absolute = step({"patch": "course", "course": str(tmp_path)}, ROOT).params.course
...
class TestRecipe:
    def test_the_base_defaults_to_vanilla(self):
        ...
        assert Recipe.from_dict({"steps": [], "base_sha1": None}, ROOT).base_sha1 is None
    ...
    def test_saving_elsewhere_keeps_paths_pointing_at_the_same_files(self, tmp_path):
        ...
        here.mkdir()
        there.mkdir()
        recipe = Recipe.from_dict({"steps": [{"patch": "course", "course": "courses/japan"}]}, here)
        ...
        assert reloaded.steps[0].params.course.resolve() == (here / "courses/japan").resolve()

    def test_course_takes_a_directory_or_holes_not_both(self, tmp_path):
        recipe = Recipe([step({"patch": "course", "course": "x", "holes": ["y"]}, tmp_path)])
        with pytest.raises(RecipeError, match=r"steps\[0\] \(course\): course takes exactly one"):
...
class TestInlineSteps:
    ...
    def test_paths_are_relative_to_the_given_directory(self, tmp_path):
        assert parse_step_arg("course:course=japan", tmp_path).params.course == tmp_path / "japan"
