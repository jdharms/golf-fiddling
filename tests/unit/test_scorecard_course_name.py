...
from golf.core.patches import COURSE_MIRRORS_PATCH, PatchError, scorecard_course_name_patch
...
class TestEncoding:
    ...
    def test_us_reproduces_the_vanilla_descriptor(self):
        ...
        us = bytes([0x6B, 0x20, 0x09, 0x01, 0x1E, 0x1C, 0x24, 0x0C, 0x18, 0x1E, 0x1B, 0x1C, 0x0E])
...
class TestTitleEncoding:
    def test_stroke_play_reproduces_the_vanilla_descriptor(self):
        ...
        vanilla = bytes(
            [0x89, 0x20, 0x0F, 0x01, 0x01, 0x08, 0x11, 0x24, 0x1C, 0x1D,
             0x1B, 0x18, 0x14, 0x0E, 0x24, 0x19, 0x15, 0x0A, 0x22]
        )
...
class TestPatch:
    ...
    def test_writes_land_in_bank_2(self):
        offsets = {leaf.name: leaf.prg_offset for leaf in scorecard_course_name_patch().patches}
        assert offsets == {
