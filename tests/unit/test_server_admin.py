...
ROUND_LINK = re.compile(r'href="/admin/submissions/(\d+)"')
...
LISTS = (
    ...
    "/admin/seeds",
    "/admin/submissions",
    "/admin/submissions?flagged=true",
    "/admin/users",
    "/admin/voided",
    "/admin/activity",
)
...
def played_seed(client: TestClient) -> tuple[str, int]:
    """A seed alice entered and recorded a round of 5s on, and that round's id; signed in as admin."""
    seed_id = entered_seed(client, "alice")
    assert client.get(scan_path(client, seed_id, "alice", strokes=5)).status_code == 200
    sign_in(client, "admin")
    [submission_id] = ROUND_LINK.findall(client.get("/admin/submissions").text)
    return seed_id, int(submission_id)
...
def test_admin_pages_are_not_found_for_anyone_else(fake_builder, who):
    with admin_client(fake_builder, strings=UNWRITTEN) as client:
        seed_id, submission_id = played_seed(client)
        client.post("/auth/logout", data={"next": "/"})
        if who:
            sign_in(client, who)
        for path in (*LISTS, f"/admin/seeds/{seed_id}", f"/admin/submissions/{submission_id}", "/admin/users/1"):
            response = client.get(path)
            assert response.status_code == 404, path
            assert "not_found.heading" in response.text, path
        for path in (f"/admin/seeds/{seed_id}/rebuild", f"/admin/submissions/{submission_id}/void"):
            assert post(client, path).status_code == 404, path
        sign_in(client, "admin")
        assert client.get(f"/admin/submissions/{submission_id}").status_code == 200
...
def test_every_list_renders_empty_and_full(fake_builder):
    with admin_client(fake_builder) as client:
        ...
        seed_id, submission_id = played_seed(client)
        post(client, f"/admin/submissions/{submission_id}/flag")
        for path in LISTS:
            page = client.get(path)
            assert page.status_code == 200, path
        assert f'href="/admin/seeds/{seed_id}"' in client.get("/admin/seeds").text
        assert f'href="/admin/submissions/{submission_id}"' in client.get("/admin/submissions?flagged=true").text
...
def test_detail_pages_show_the_seed_the_round_and_the_player(fake_builder):
    with admin_client(fake_builder) as client:
        ...
        round_page = client.get(f"/admin/submissions/{submission_id}").text
    ...
    assert f'href="/admin/submissions/{submission_id}"' in seed_page
    ...
    assert f'href="/admin/submissions/{submission_id}"' in user_page


@pytest.mark.parametrize("path", ["/admin/seeds/0000000001", "/admin/submissions/99", "/admin/users/99"])
...
def test_the_activity_page_and_histories_show_who_did_what(fake_builder):
    with admin_client(fake_builder) as client:
        seed_id, submission_id = played_seed(client)
        post(client, f"/admin/submissions/{submission_id}/flag", note="five on every hole?")
    ...
    assert f'href="/admin/submissions/{submission_id}"' in activity
...
def test_a_restored_rounds_history_reaches_back_past_the_void(fake_builder):
    with admin_client(fake_builder) as client:
        seed_id, submission_id = played_seed(client)
        post(client, f"/admin/submissions/{submission_id}/flag", note="five on every hole?")
        post(client, f"/admin/submissions/{submission_id}/void", note="warm-up")
        [restore_path] = re.findall(r'action="(/admin/voided/\d+/restore)"', client.get("/admin/voided").text)
...
def test_a_voided_rounds_page_names_the_admin_who_voided_it(fake_builder):
    with admin_client(fake_builder) as client:
        _seed_id, submission_id = played_seed(client)
        post(client, f"/admin/submissions/{submission_id}/void")
        voided = client.get("/admin/voided").text
        activity = client.get("/admin/activity").text
    assert "by admin" in voided
    # the round is no longer a submission, so activity points at the voided page instead
...
def test_flagging_marks_the_round_publicly_and_keeps_the_note_private(fake_builder):
    with admin_client(fake_builder, strings=UNWRITTEN) as client:
        seed_id, submission_id = played_seed(client)
        response = post(client, f"/admin/submissions/{submission_id}/flag", note="five on every hole?")
        assert response.status_code == 303
        assert response.headers["location"] == f"/admin/submissions/{submission_id}?result=flagged"
        assert "five on every hole?" in client.get(f"/admin/submissions/{submission_id}").text
        seed_page = client.get(f"/h/{seed_id}").text
        sign_in(client, "alice")
        my_page = client.get("/me").text
        sign_in(client, "admin")
        post(client, f"/admin/submissions/{submission_id}/unflag")
...
def test_voiding_removes_the_round_and_refuses_its_scan_until_restored(fake_builder):
    with admin_client(fake_builder, strings=UNWRITTEN) as client:
        seed_id, submission_id = played_seed(client)
        response = post(client, f"/admin/submissions/{submission_id}/void", note="warm-up")
        ...
        [restore_path] = re.findall(r'action="(/admin/voided/\d+/restore)"', voided)

        rescan = client.get(scan_path(client, seed_id, "alice", strokes=5))
        assert rescan.status_code == 404
        assert "submission.rejected.unrecognized" in rescan.text

        restored = post(client, restore_path)
        assert re.fullmatch(r"/admin/submissions/\d+\?result=restored", restored.headers["location"])
        assert client.get(restored.headers["location"]).status_code == 200
        assert "submission.recorded.heading_earlier" in client.get(scan_path(client, seed_id, "alice", strokes=5)).text
...
def test_restoring_into_a_slot_with_a_round_is_refused(fake_builder):
    with admin_client(fake_builder, strings=UNWRITTEN) as client:
        ...
        response = post(client, "/admin/voided/1/restore")
...
@pytest.mark.parametrize("action", ["flag", "unflag", "void"])
def test_acting_on_a_missing_round_is_not_found(fake_builder, action):
    with admin_client(fake_builder) as client:
        sign_in(client, "admin")
        assert post(client, f"/admin/submissions/99/{action}").status_code == 404
