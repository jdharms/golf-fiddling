...
ME = {"id": "80351110224678912", "username": "nelly", "global_name": "Nelly", "avatar": "8342729096ea3675442027381ff50dfe"}
...
def discord(token=None, me=None, token_status=200, me_status=200, seen=None):
    ...
    def handle(request: httpx2.Request) -> httpx2.Response:
        ...
        if request.url.path == "/api/v10/oauth2/token":
            body = token if token is not None else {"access_token": "token-abc", "token_type": "Bearer"}
    ...
    return DiscordClient("client-id", "client-secret", transport=httpx2.MockTransport(handle))
...
def test_the_authorize_url_asks_for_identify_with_the_state_and_redirect():
    url = urlsplit(DiscordClient("client-id", "secret").authorize_url("state-123", REDIRECT))
...
def test_identify_exchanges_the_code_and_reads_the_user():
    ...
    assert token_request.headers["authorization"] == "Basic " + base64.b64encode(b"client-id:client-secret").decode()
...
def test_a_user_without_a_display_name_or_avatar_is_accepted():
    identity = identify(discord(me={"id": "1", "username": "nelly", "global_name": None, "avatar": None}))
...
@pytest.mark.parametrize(
    "value", [None, "", "h/1", "//evil.example/", "/\\evil.example", "https://evil.example/", "javascript:alert(1)"]
)
